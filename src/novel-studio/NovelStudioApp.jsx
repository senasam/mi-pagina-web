...
function CharacterNetworkPage({ novel, structure, entries, setEntries, aiSettings, readOnly, setSaveState }) {
  ...
  const aiConfigured = aiSettings?.provider === "chatgpt-manual" || (aiSettings?.provider === "ollama" ? Boolean(aiSettings.ollamaModel) : Boolean(aiSettings?.apiKey));
  useEffect(() => {
...
function SettingsPage({ novel, setNovel, structure, preferences, setPreferences, readOnly, setSaveState }) {
  ...
  return <section className="studio-page studio-settings-page"><PageTitle kicker="Proyecto y portabilidad" title="Configuración" text="Administra cada parte de la novela en un espacio más simple y ordenado.">{activeTab === "edicion" && <a className="studio-button studio-button--secondary" href={studioHref(novel.id, "libro")} onClick={(event) => handleStudioLink(event, studioHref(novel.id, "libro"))}><BookOpen size={17} /> Ver libro</a>}</PageTitle>
    ...
    <div className="studio-settings-content" id={`settings-panel-${activeTab}`} role="tabpanel" aria-labelledby={`settings-tab-${activeTab}`}>
      ...
      {activeTab === "ia" && <div className="studio-settings-single"><AiSettingsPanel aiDraft={aiDraft} setAiDraft={setAiDraft} savedAi={preferences.ai} readOnly={readOnly} onSave={saveAi} onMessage={setMessage} /></div>}
...
function AiSettingsPanel({ aiDraft, setAiDraft, savedAi, readOnly, onSave, onMessage }) {
  ...
  return <section className="studio-panel studio-ai-settings"><div className="studio-panel-heading"><div><p className="studio-kicker">Workspace completo</p><h2>Inteligencia artificial</h2></div><Sparkles size={24} /></div><p>Elige cómo generar títulos y resúmenes. La selección se comparte entre todas las novelas de esta carpeta.</p>
    ...
    <section className="studio-ai-action-center" aria-labelledby="studio-ai-actions-title"><div className="studio-ai-action-center__heading"><div><strong id="studio-ai-actions-title">Acciones de IA</strong><small>{aiDraft.provider === "ollama" ? "Comprueba o instala el modelo, prueba la conexión y guarda los cambios." : "Prueba la configuración y guarda los cambios cuando esté lista."}</small></div><span>{aiDraft.provider === "ollama" ? "Ollama local" : aiDraft.provider === "openai" ? "OpenAI API" : "ChatGPT manual"}</span></div><div className="studio-ai-action-center__buttons"><div className="studio-ai-action-center__tools">{aiDraft.provider === "ollama" && <><button type="button" className="studio-button studio-button--secondary" disabled={detecting} onClick={detectOllama}>{detecting ? "Detectando…" : "Comprobar instalación"}</button>{ollamaModels.length > 0 && <button type="button" className="studio-button studio-button--secondary" disabled={readOnly || detecting} aria-expanded={showModelInstaller} aria-controls="ollama-additional-models" onClick={() => setShowModelInstaller((value) => !value)}><Download size={17} /> {showModelInstaller ? "Cerrar instalador" : "Instalar otro modelo"}</button>}</>}<button type="button" className="studio-button studio-button--secondary" disabled={!providerReady || testing} onClick={testProvider}><Sparkles size={17} /> {testing ? "Probando…" : aiDraft.provider === "chatgpt-manual" ? "Abrir ChatGPT" : "Probar conexión"}</button>{savedAi.apiKey && aiDraft.provider === "openai" && <button type="button" className="studio-button studio-button--danger" disabled={readOnly} onClick={removeKey}>Eliminar clave</button>}</div><button type="button" className="studio-button studio-ai-action-center__save" disabled={readOnly || !providerReady} onClick={() => onSave()}><Save size={17} /> Guardar IA</button></div></section>
