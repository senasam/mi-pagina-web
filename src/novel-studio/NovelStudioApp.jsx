...
import { hasSessionCredential, removeSessionCredential, saveSessionCredential } from "../platform/ai/credentialClient.js";
...
function SettingsPage({ novel, setNovel, structure, preferences, setPreferences, readOnly, setSaveState }) {
  ...
  const saveAi = async (nextAi = aiDraft, sessionSecret = "", activate = true) => {
    ...
    try {
      ...
      if (nextAi.provider === AI_PROVIDERS.OPENAI && sessionSecret.trim()) {
        credentialRef = saveSessionCredential({ toolId: AI_TOOL_IDS.NOVEL_STUDIO, provider: AI_PROVIDERS.OPENAI, secret: sessionSecret });
      }
...
function AiSettingsPanel({ aiDraft, setAiDraft, readOnly, onSave, onMessage }) {
  ...
  const removeKey = async () => {
    if (!confirm("¿Eliminar la clave de OpenAI de esta sesión?")) return;
  ...
  return <section className="studio-panel studio-ai-settings"><div className="studio-panel-heading"><div><p className="studio-kicker">Workspace completo</p><h2>Inteligencia artificial</h2></div><Sparkles size={24} /></div><p>Elige cómo generar títulos y resúmenes. La selección se comparte entre todas las novelas de esta carpeta.</p>
    ...
    {aiDraft.provider === "openai" && <><label>Clave API de OpenAI<div className="studio-secret-field"><input type={showKey ? "text" : "password"} autoComplete="off" spellCheck="false" placeholder={sessionHasKey ? "Clave disponible en esta sesión" : "sk-…"} value={apiKey} disabled={readOnly} onChange={(event) => setApiKey(event.target.value)} /><button type="button" className="studio-button studio-button--secondary" onClick={() => setShowKey((value) => !value)}>{showKey ? "Ocultar" : "Mostrar"}</button></div></label><label>Modelo<select value={aiDraft.model} disabled={readOnly} onChange={(event) => update({ model: event.target.value })}><option value="gpt-5.6-luna">GPT-5.6 Luna · menor costo</option><option value="gpt-5.6-terra">GPT-5.6 Terra · equilibrado</option><option value="gpt-5.6-sol">GPT-5.6 Sol · máxima capacidad</option></select></label><div className="studio-local-secret-warning"><strong>Credencial temporal</strong><p>La clave vive sólo en la memoria de esta pestaña. No se escribe en <code>preferences.json</code> ni se incluye en respaldos ZIP; deberás introducirla de nuevo al cerrar o recargar.</p></div></>}
