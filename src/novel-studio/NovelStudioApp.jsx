...
import {
  OLLAMA_MODEL_PRESETS, buildChatGptManualPrompt, listOllamaModels, normalizeChatGptUrl,
...
function WritePage({ novel, structure, setStructure, codex, aiSettings, readOnly, setSaveState }) {
  ...
  const aiHint = !aiSettings?.enabled || !aiConfigured
...
function AiSettingsPanel({ aiDraft, setAiDraft, savedAi, readOnly, onSave, onMessage }) {
  ...
  const detectOllama = async () => {
    ...
    try {
      ...
      const chosenModel = aiDraft.ollamaModel || models[0]?.name || "";
      ...
      if (!aiDraft.ollamaModel && chosenModel) update({ ollamaModel: chosenModel, enabled: false });
    ...
    } catch (error) { setOllamaModels([]); setOllamaStatus("unavailable"); onMessage(error.message); setStatusDialog({ kind: "detect", state: "error", provider: "Ollama local", model: aiDraft.ollamaModel || "Sin seleccionar", title: "No se pudo comprobar Ollama", message: error.message }); }
  ...
  const testProvider = async () => {
    ...
    } catch (error) { setStatusDialog({ kind: "test", state: "error", provider, model, title: "Falló la prueba de conexión", message: error.message }); }
  ...
  return <section className="studio-panel studio-ai-settings"><div className="studio-panel-heading"><div><p className="studio-kicker">Workspace completo</p><h2>Inteligencia artificial</h2></div><Sparkles size={24} /></div><p>Elige cómo generar títulos y resúmenes. La selección se comparte entre todas las novelas de esta carpeta.</p>
    <label>Proveedor<select value={aiDraft.provider} disabled={readOnly} onChange={(event) => update({ provider: event.target.value, enabled: false })}><option value="chatgpt-manual">ChatGPT manual</option><option value="ollama">Ollama · modelos locales</option><option value="openai">OpenAI API</option></select></label>
    ...
    <label className="studio-check-label"><input type="checkbox" checked={aiDraft.enabled} disabled={readOnly || !providerReady} onChange={(event) => update({ enabled: event.target.checked })} /> Habilitar botones de IA</label>
    ...
    {statusDialog && <AiStatusDialog dialog={statusDialog} onClose={() => setStatusDialog(null)} onCancel={statusDialog.kind === "install" && statusDialog.state === "working" ? () => installControllerRef.current?.abort() : null} />}
  </section>;
}
...
function OllamaSetupWizard({ aiDraft, update, installMode, setInstallMode, originCommand, onCopy, onInstall }) {
  ...
  return <div className="studio-ollama-wizard">
    ...
    {installMode && aiDraft.ollamaModel && <div className="studio-install-steps"><h3>3. Instala y conecta, en este orden</h3><ol className="studio-setup-sequence"><li><strong>Descarga e instala Ollama.</strong><p>{installMode === "desktop" ? "Pulsa el botón, abre el archivo OllamaSetup.exe descargado y termina la instalación. Si Ollama ya está instalado, puedes omitir este punto." : "Abre la guía oficial y completa la instalación de la CLI antes de continuar."}</p><a className="studio-button studio-button--secondary" href={installMode === "desktop" ? "https://ollama.com/download/windows" : "https://docs.ollama.com/windows#standalone-cli"} target="_blank" rel="noreferrer">{installMode === "desktop" ? "A. Descargar Ollama para Windows" : "A. Abrir guía de instalación CLI"}</a></li><li><strong>Autoriza esta página una sola vez.</strong><p>Pulsa <strong>Copiar</strong>, abre PowerShell normalmente, pega el comando y presiona <kbd>Enter</kbd>. No necesitas abrirlo como administrador.</p><div className="studio-command"><code>{originCommand}</code><button type="button" onClick={() => onCopy(originCommand)}>Copiar</button></div></li><li><strong>Reinicia Ollama.</strong><p>Ciérralo desde el icono de Ollama junto al reloj de Windows y vuelve a abrirlo desde el menú Inicio.</p></li><li><strong>Descarga el modelo elegido.</strong><p>Con Ollama abierto, este botón instalará <strong>{aiDraft.ollamaModel}</strong> en el PC y mostrará el progreso.</p><button type="button" className="studio-button" onClick={onInstall}><Download size={17} /> D. Instalar {aiDraft.ollamaModel}</button></li></ol><details><summary>Alternativa: instalar el modelo desde PowerShell</summary><div className="studio-command"><code>{pullCommand}</code><button type="button" onClick={() => onCopy(pullCommand)}>Copiar</button></div></details></div>}
  </div>;
}

function AiStatusDialog({ dialog, onClose, onCancel }) {
  const working = dialog.state === "working";
  return <div className="studio-modal" role="dialog" aria-modal="true" aria-labelledby="ai-status-title"><div className={`studio-modal__card studio-status-dialog is-${dialog.state}`}><p className="studio-kicker"><Sparkles size={15} /> Estado de IA</p><h2 id="ai-status-title">{dialog.title}</h2><dl><div><dt>Proveedor</dt><dd>{dialog.provider}</dd></div><div><dt>Modelo</dt><dd>{dialog.model}</dd></div></dl>{dialog.kind === "install" && <div className="studio-install-progress"><progress max="100" value={dialog.percent || 0} /><strong>{dialog.percent || 0}%</strong></div>}<p>{dialog.message}</p><div className="studio-row">{onCancel && <button type="button" className="studio-button studio-button--danger" onClick={onCancel}>Cancelar descarga</button>}<button type="button" className="studio-button" disabled={working} onClick={onClose}>{working ? "Espera…" : "Cerrar"}</button></div></div></div>;
}
