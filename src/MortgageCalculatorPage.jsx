...
import { ArrowLeft, ArrowRight, Calculator, Check, Clipboard, Copy, Lock, Plus, Printer, RefreshCw, RotateCcw, Trash2, Unlock } from "lucide-react";
...
const mortgageWizardSteps = [
  { number: 1, label: "Propiedad" },
  { number: 2, label: "Recursos" },
  { number: 3, label: "Crédito" },
  { number: 4, label: "Seguros" },
  { number: 5, label: "Gastos iniciales" },
  { number: 6, label: "Corretaje" },
  { number: 7, label: "Revisión" },
];
const wizardErrorStep = (key) => {
  if (["uf", "propertyPrice", "appraisal", "financing", "availableDownPayment"].includes(key)) return 1;
  if (["householdIncome", "existingDebt", "burden"].includes(key)) return 2;
  if (["annualRate", "rateConvention", "termYears", "termExtraMonths"].includes(key)) return 3;
  if (["insuredBorrowers", "lifeInsuranceRate", "lifeInsuranceBase", "propertyInsuranceRate", "propertyInsuranceBase", "otherRecurring"].includes(key)) return 4;
  if (["legalExpense", "appraisalExpense", "deedDraftExpense", "notaryExpense", "conservatorRate", "otherExpense", "priorDfl2Homes"].includes(key)) return 5;
  return 6;
};
...
function Field({ id, label, unit, value, onChange, error, help, type = "text", children, min = 0, max = 100, step = "0.01" }) {
  const [sliderLocked, setSliderLocked] = useState(true);
  const isPercentage = !children && String(unit || "").startsWith("%");
  const numericValue = Number.isFinite(n(value)) ? Math.min(max, Math.max(min, n(value))) : min;
  return <div className={`mortgage-field${error ? " mortgage-field--error" : ""}`}>
    <label htmlFor={id}>{label} {unit && <span>({unit})</span>}</label>
    {children || (isPercentage ? <div className="percentage-slider"><input id={id} type="range" min={min} max={max} step={step} value={numericValue} aria-valuetext={`${formatNumericInput(value)}%`} aria-invalid={Boolean(error)} aria-describedby={`${id}-help${error ? ` ${id}-error` : ""}`} onChange={(event) => onChange(event.target.value)} disabled={sliderLocked} data-private="true" /><output htmlFor={id}>{formatNumericInput(value)}%</output><button type="button" className={`slider-lock ${sliderLocked ? "slider-lock--locked" : "slider-lock--unlocked"}`} onClick={() => setSliderLocked((locked) => !locked)} aria-pressed={!sliderLocked} aria-label={`${sliderLocked ? "Desbloquear" : "Bloquear"} ${label}`} title={sliderLocked ? "Desbloquear slider" : "Bloquear slider"}>{sliderLocked ? <Lock size={17} /> : <Unlock size={17} />}<span className="sr-only">{sliderLocked ? "Desbloquear" : "Bloquear"}</span></button></div> : <input id={id} type={type} inputMode={type === "text" ? "decimal" : undefined} value={formatNumericInput(value)} onChange={(event) => onChange(normalizeNumericInput(event.target.value))} aria-invalid={Boolean(error)} aria-describedby={`${id}-help${error ? ` ${id}-error` : ""}`} data-private="true" autoComplete="off" />)}
    {help && <small id={`${id}-help`}>{help}</small>}
    {error && <small className="field-error" id={`${id}-error`}>{error}</small>}
  </div>;
}

function MoneyField({ id, label, valueUf, onChangeUf, currencyMode, ufValue, error, help, optional = false }) {
  const parsedUf = n(valueUf);
  const displayValue = valueUf === "" ? "" : currencyMode === "clp" && ufValue > 0 ? Math.round(parsedUf * ufValue) : valueUf;
  const change = (value) => {
    if (value === "") return onChangeUf("");
    const parsed = n(value);
    onChangeUf(currencyMode === "clp" && ufValue > 0 ? String(parsed / ufValue) : value);
  };
  const equivalent = valueUf !== "" && Number.isFinite(parsedUf) && ufValue > 0
    ? currencyMode === "clp" ? formatUf(parsedUf) : formatClp(parsedUf * ufValue)
    : null;
  return <Field id={id} label={label} unit={`${currencyMode === "clp" ? "CLP" : "UF"}${optional ? ", opcional" : ""}`} value={displayValue} onChange={change} error={error} help={<>{equivalent && <span className="money-equivalent">Equivale a {equivalent}. </span>}{help}</>} />;
}
...
export default function MortgageCalculatorPage() {
  ...
  const displayMoney = (valueUf) => currencyMode === "clp" && effectiveUf > 0 ? formatClp(valueUf * effectiveUf) : formatUf(valueUf);
  const equivalentMoney = (valueUf) => currencyMode === "clp" ? formatUf(valueUf) : effectiveUf > 0 ? formatClp(valueUf * effectiveUf) : null;
  ...
  const goToStep = (step) => {
    ...
    setActiveStep(step);
    setResult(null);
    requestAnimationFrame(() => document.getElementById("mortgage-wizard")?.scrollIntoView({ block: "start", behavior: "smooth" }));
  };
  const continueWizard = () => {
    const currentErrors = Object.fromEntries(Object.entries(validate(form, effectiveUf, mode)).filter(([key]) => wizardErrorStep(key) === activeStep));
    setErrors(currentErrors);
    if (Object.keys(currentErrors).length) {
      setNotice("Revisa los campos indicados antes de continuar.");
      requestAnimationFrame(() => document.getElementById("mortgage-errors")?.focus());
      return;
    }
    const nextStep = Math.min(mortgageWizardSteps.length, activeStep + 1);
  ...
  const calculate = () => {
    ...
    if (Object.keys(nextErrors).length) {
      const firstErrorStep = wizardErrorStep(Object.keys(nextErrors)[0]);
      setActiveStep(firstErrorStep);
      setHighestStep((current) => Math.max(current, firstErrorStep));
      setNotice("Revisa los campos indicados antes de calcular.");
      requestAnimationFrame(() => document.getElementById("mortgage-errors")?.focus());
      return;
    }
    try {
  ...
  const reset = () => { setForm(cloneForm(initialForm)); setResult(null); setErrors({}); setScenarios([]); setBaselineId(null); setActiveStep(1); setHighestStep(1); setNotice("La calculadora y los escenarios locales se reiniciaron."); trackEvent("mortgage_tool_reset", { category: mode }); };
  ...
  return <PageShell><main id="main-content" className="content-page mortgage-page"><Breadcrumbs items={breadcrumbs} />
    <header className="mortgage-hero"><div><p className="eyebrow">Finanzas personales · herramienta educativa</p><h1>¿Cuánto costaría financiar esta propiedad?</h1><p>Calcula el dividendo, el pie, los gastos iniciales y la carga mensual de un crédito hipotecario en UF.</p>
      <section className="monetary-settings mortgage-monetary-settings" aria-labelledby="uf-title" aria-label="UF de la simulación">
        <div className="monetary-settings__body">
          ...
          <fieldset className="currency-selector">
            ...
            <div>
              <label className={currencyMode === "uf" ? "active" : ""}><input type="radio" name="mortgage-currency-mode" value="uf" checked={currencyMode === "uf"} onChange={() => setCurrencyMode("uf")} /><span><strong>UF</strong><small>Unidad de Fomento</small></span></label>
              <label className={currencyMode === "clp" ? "active" : ""}><input type="radio" name="mortgage-currency-mode" value="clp" checked={currencyMode === "clp"} onChange={() => setCurrencyMode("clp")} /><span><strong>CLP</strong><small>Pesos chilenos</small></span></label>
            </div>
          </fieldset>
        </div>
        <p className="monetary-settings__note"><strong>Estimación educativa.</strong> La unidad elegida cambia cómo ingresas y lees los montos, no los supuestos financieros. Confirma tasas, seguros, gastos, impuestos y condiciones.</p>
      </section>
    </div></header>

    <section className="mortgage-workspace" aria-labelledby="calculator-title">
      <div className="mortgage-workspace__intro"><p className="eyebrow">Configura tu escenario</p><h2 id="calculator-title">{mode === "mortgage" ? "¿Cuánto sería el dividendo y cuánto dinero inicial necesitaría?" : "¿Qué valor de propiedad podría explorar con este ingreso y presupuesto mensual?"}</h2><p>Completa los supuestos de la operación. Puedes ajustar cada valor y volver a calcular cuantas veces necesites.</p></div>
      <div className="mode-switch" role="tablist" aria-label="Modo de cálculo"><button role="tab" aria-selected={mode === "mortgage"} className={mode === "mortgage" ? "active" : ""} onClick={() => { setMode("mortgage"); setResult(null); setActiveStep(1); setHighestStep(1); setErrors({}); }}>Estimar un crédito</button><button role="tab" aria-selected={mode === "affordability"} className={mode === "affordability" ? "active" : ""} onClick={() => { setMode("affordability"); setResult(null); setActiveStep(1); setHighestStep(1); setErrors({}); }}>Estimar valor compatible</button></div>
      <nav id="mortgage-wizard" className="mortgage-wizard" aria-label="Pasos de la calculadora">
        <ol className="wizard-stepper">{mortgageWizardSteps.map((step) => <li key={step.number} className={activeStep === step.number ? "active" : step.number < highestStep ? "complete" : step.number === highestStep ? "available" : ""}><button type="button" disabled={step.number > highestStep} aria-current={activeStep === step.number ? "step" : undefined} onClick={() => goToStep(step.number)}><span>{activeStep === step.number ? step.number : step.number < highestStep ? <Check size={15} /> : step.number}</span><strong>{step.label}</strong></button></li>)}</ol>
        <p>Paso {activeStep} de {mortgageWizardSteps.length} · {mortgageWizardSteps[activeStep - 1].label}</p>
      </nav>
      ...
      <form onSubmit={(event) => { event.preventDefault(); if (activeStep < mortgageWizardSteps.length) continueWizard(); else calculate(); }} noValidate>
        <fieldset hidden={activeStep !== 1}><legend>Propiedad</legend><p>El menor valor entre precio y tasación se usa como base financiable. La institución puede aplicar criterios propios para determinar el valor financiable y el porcentaje máximo.</p><div className="mortgage-grid">
          {mode === "mortgage" && <MoneyField id="propertyPrice" label="Valor comercial" valueUf={form.propertyPrice} onChangeUf={(value) => update("propertyPrice", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.propertyPrice} help="Ejemplo: 4.000 UF." />}
          <MoneyField id="appraisal" label="Tasación disponible" valueUf={form.appraisal} onChangeUf={(value) => update("appraisal", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.appraisal} optional help="Déjalo vacío si aún no existe." />
          <Field id="financing" label="Financiamiento" unit="%" value={form.financing} onChange={(value) => update("financing", value)} error={errors.financing} min={1} max={100} step="1" help="Supuesto de simulación, no política universal." />
          <MoneyField id="availableDownPayment" label="Pie disponible" valueUf={form.availableDownPayment} onChangeUf={(value) => { update("availableDownPayment", value); update("availableDownPaymentUnit", "uf"); }} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.availableDownPayment} optional help="Permite compararlo con el pie mínimo calculado." />
        </div></fieldset>
        <fieldset hidden={activeStep !== 2}><legend>Recursos y endeudamiento</legend><div className="mortgage-grid"><Field id="householdIncome" label="Ingreso mensual del hogar" unit="CLP" value={form.householdIncome} onChange={(value) => update("householdIncome", value)} error={errors.householdIncome} /><Field id="existingDebt" label="Obligaciones mensuales existentes" unit="CLP" value={form.existingDebt} onChange={(value) => update("existingDebt", value)} error={errors.existingDebt} /><Field id="burden" label="Umbral de carga para planificar" unit="%" value={form.burden} onChange={(value) => update("burden", value)} error={errors.burden} min={1} max={100} step="1" help="Es una decisión de planificación; no predice criterios de aprobación." /></div></fieldset>
        <fieldset hidden={activeStep !== 3}><legend>Crédito</legend><div className="mortgage-grid"><Field id="annualRate" label="Tasa anual" unit="%" value={form.annualRate} onChange={(value) => update("annualRate", value)} error={errors.annualRate} min={0} max={20} step="0.01" /><Field id="rateConvention" label="Convención de tasa" value={form.rateConvention} error={errors.rateConvention}><select id="rateConvention" value={form.rateConvention} onChange={(event) => update("rateConvention", event.target.value)}><option value={RATE_CONVENTIONS.EFFECTIVE}>Anual efectiva, ajuste 365/360</option><option value={RATE_CONVENTIONS.NOMINAL_MONTHLY}>Nominal anual, capitalización mensual</option></select></Field><Field id="termYears" label="Plazo" unit="años" value={form.termYears} onChange={(value) => update("termYears", value)} error={errors.termYears} /><Field id="termExtraMonths" label="Meses adicionales" unit="0 a 11" value={form.termExtraMonths} onChange={(value) => update("termExtraMonths", value)} error={errors.termYears} /></div><p className="input-note">La tasa anual efectiva se ajusta primero a base 365/360 y luego se convierte a su equivalente mensual con raíz doce. La nominal mensual se divide por doce. La frecuencia modelada es mensual y no incluye meses de gracia.</p></fieldset>
        <fieldset hidden={activeStep !== 4}><legend>Seguros y otros pagos mensuales</legend><p>Son supuestos editables, no primas universales. Elige la base usada y confirma con la institución la base contractual, cobertura, tasa y número de asegurados.</p><div className="mortgage-grid"><Field id="insuredBorrowers" label="Personas aseguradas" unit="número" value={form.insuredBorrowers} onChange={(value) => update("insuredBorrowers", value)} error={errors.insuredBorrowers} /><Field id="lifeInsuranceRate" label="Tasa mensual de desgravamen por persona" unit="%" value={form.lifeInsuranceRate} onChange={(value) => update("lifeInsuranceRate", value)} error={errors.lifeInsuranceRate} min={0} max={1} step="0.0001" /><Field id="lifeInsuranceBase" label="Base del desgravamen" value={form.lifeInsuranceBase}><select id="lifeInsuranceBase" value={form.lifeInsuranceBase} onChange={(event) => update("lifeInsuranceBase", event.target.value)}><option value="opening-loan-balance">Saldo inicial de cada mes</option><option value="initial-loan-principal">Capital inicial</option></select></Field><Field id="propertyInsuranceRate" label="Tasa mensual incendio y sismo" unit="%" value={form.propertyInsuranceRate} onChange={(value) => update("propertyInsuranceRate", value)} error={errors.propertyInsuranceRate} min={0} max={1} step="0.0001" /><Field id="propertyInsuranceBase" label="Base del seguro del inmueble" value={form.propertyInsuranceBase}><select id="propertyInsuranceBase" value={form.propertyInsuranceBase} onChange={(event) => update("propertyInsuranceBase", event.target.value)}><option value="property-value">Valor comercial</option><option value="appraisal-value">Tasación ingresada</option><option value="initial-loan-principal">Capital inicial</option></select></Field><MoneyField id="otherRecurring" label="Otros pagos mensuales" valueUf={form.otherRecurring} onChangeUf={(value) => update("otherRecurring", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.otherRecurring} /></div></fieldset>
        <fieldset hidden={activeStep !== 5}><legend>Inicio: gastos operacionales</legend><p>Valores iniciales editables según el esquema proporcionado. El impuesto al mutuo se calcula sobre el capital financiado; el Conservador, sobre el valor comercial.</p><div className="mortgage-grid"><MoneyField id="appraisalExpense" label="Tasación de la propiedad" valueUf={form.appraisalExpense} onChangeUf={(value) => update("appraisalExpense", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.appraisalExpense} /><MoneyField id="legalExpense" label="Estudio de títulos" valueUf={form.legalExpense} onChangeUf={(value) => update("legalExpense", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.legalExpense} /><MoneyField id="deedDraftExpense" label="Borrador de escritura" valueUf={form.deedDraftExpense} onChangeUf={(value) => update("deedDraftExpense", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.deedDraftExpense} /><MoneyField id="notaryExpense" label="Notaría" valueUf={form.notaryExpense} onChangeUf={(value) => update("notaryExpense", value)} currencyMode={currencyMode} ufValue={effectiveUf} error={errors.notaryExpense} /><Field id="conservatorRate" label="Conservador de Bienes Raíces" unit="% del valor comercial" value={form.conservatorRate} onChange={(value) => update("conservatorRate", value)} error={errors.conservatorRate} min={0} max={5} step="0.01" /><Field id="otherExpense" label="Otros gastos confirmados" unit={form.otherExpenseMode === "fixed-uf" ? "UF" : form.otherExpenseMode === "fixed-clp" ? "CLP" : "%"} value={form.otherExpense} onChange={(value) => update("otherExpense", value)} error={errors.otherExpense} min={0} max={100} step="0.01" /><Field id="otherExpenseMode" label="Base de otros gastos" value={form.otherExpenseMode}><select id="otherExpenseMode" value={form.otherExpenseMode} onChange={(event) => update("otherExpenseMode", event.target.value)}><option value="fixed-uf">Monto fijo en UF</option><option value="fixed-clp">Monto fijo en CLP</option><option value="property-percentage">Porcentaje del valor comercial</option><option value="principal-percentage">Porcentaje del capital</option></select></Field></div><div className="dfl2-panel"><h3>Beneficio DFL2 para el impuesto al mutuo</h3><p>La tasa baja de 0,8% a 0,2% solo cuando se cumplen conjuntamente las condiciones indicadas. Confírmalas en la escritura, el certificado de recepción y con la institución.</p><div className="dfl2-checklist"><label><input type="checkbox" checked={form.dfl2Certified} onChange={(event) => update("dfl2Certified", event.target.checked)} /> La vivienda está formalmente acogida al DFL2</label><label><input type="checkbox" checked={form.dfl2FirstTransfer} onChange={(event) => update("dfl2FirstTransfer", event.target.checked)} /> Es la primera transferencia de la vivienda</label><label><input type="checkbox" checked={form.dfl2WithinTwoYears} onChange={(event) => update("dfl2WithinTwoYears", event.target.checked)} /> Ocurre dentro de 2 años desde la recepción municipal</label><label><input type="checkbox" checked={form.buyerNaturalPerson} onChange={(event) => update("buyerNaturalPerson", event.target.checked)} /> Compra una persona natural</label></div><Field id="priorDfl2Homes" label="Viviendas que ya ocupan el cupo DFL2 del comprador" unit="número" value={form.priorDfl2Homes} onChange={(value) => update("priorDfl2Homes", value)} error={errors.priorDfl2Homes} help="Con 0 o 1 hay cupo disponible; con 2 o más se usa la tasa general." /><p className="dfl2-result"><strong>Tasa modelada:</strong> {evaluateDfl2StampTax({ certifiedProperty: form.dfl2Certified, firstTransfer: form.dfl2FirstTransfer, withinTwoYearsOfReception: form.dfl2WithinTwoYears, naturalPersonBuyer: form.buyerNaturalPerson, priorBenefitedHomes: n(form.priorDfl2Homes) }).eligible ? "0,2% (beneficio DFL2)" : "0,8% (tasa general)"}</p></div></fieldset>
        <div className="mortgage-wizard-panel" hidden={activeStep !== 6}><BrokerageInputSection form={form} update={update} errors={errors} preview={brokeragePreview} ufValueClp={effectiveUf} resetBrokerage={resetBrokerage} /><BrokerageComparisonTool propertyPriceUf={result?.propertyPriceUf || n(form.propertyPrice)} ufValueClp={effectiveUf} /></div>
        <fieldset className="mortgage-review" hidden={activeStep !== 7}><legend>Revisa antes de calcular</legend><p>Estos son los supuestos principales. Puedes volver a cualquier paso para corregirlos.</p><dl><div><dt>Propiedad</dt><dd>{mode === "mortgage" ? displayMoney(n(form.propertyPrice)) : "Valor por estimar"} · {formatNumericInput(form.financing)}% financiado</dd></div><div><dt>Recursos</dt><dd>{formatClp(n(form.householdIncome))} de ingreso · {formatNumericInput(form.burden)}% de umbral</dd></div><div><dt>Crédito</dt><dd>{formatNumericInput(form.annualRate)}% anual · {formatNumericInput(n(form.termYears) * 12 + n(form.termExtraMonths))} meses</dd></div><div><dt>Seguros</dt><dd>{formatNumericInput(form.insuredBorrowers)} persona(s) · otros pagos {displayMoney(n(form.otherRecurring))}</dd></div><div><dt>Gastos declarados</dt><dd>Tasación {displayMoney(n(form.appraisalExpense))} · notaría {displayMoney(n(form.notaryExpense))}</dd></div><div><dt>Corretaje</dt><dd>{form.brokerParticipation === "no" ? "Sin corredor" : form.buyerCommissionStatus === "yes" ? "Comisión ingresada" : "Condición pendiente de confirmar"}</dd></div></dl><p className="mortgage-review__note">El resultado seguirá siendo una estimación educativa. Revisa la oferta y los documentos de la institución antes de tomar una decisión.</p></fieldset>
        <div className="calculator-actions mortgage-wizard-actions">{activeStep > 1 && <button className="button button--secondary" type="button" onClick={() => goToStep(activeStep - 1)}><ArrowLeft size={17} /> Anterior</button>}{activeStep < mortgageWizardSteps.length ? <button className="button button--primary" type="button" onClick={continueWizard}>Continuar <ArrowRight size={17} /></button> : <button className="button button--primary" type="submit"><Calculator size={17} /> {mode === "mortgage" ? "Calcular escenario" : "Estimar valor orientativo"}</button>}<button className="button button--secondary mortgage-wizard-reset" type="button" onClick={reset}><RotateCcw size={17} /> Reiniciar</button></div>
      </form>
    </section>
    ...
    {result && <section className="mortgage-results" ref={resultRef} tabIndex="-1" aria-labelledby="results-title"><div className="section-title"><p className="eyebrow">Resultado de planificación</p><h2 id="results-title">{mode === "affordability" ? "Valor de propiedad orientativo bajo este escenario" : "Resumen del escenario"}</h2><p>Resultados calculados con la UF y los supuestos mostrados. Los seguros, gastos y equivalencias en pesos deben confirmarse.</p></div>
      {mode === "affordability" && <div className="headline-result"><span>Valor orientativo</span><strong>{displayMoney(result.propertyPriceUf)}</strong><small>{equivalentMoney(result.propertyPriceUf)} con la UF seleccionada</small></div>}
      <dl className="metrics-grid"><MoneyMetric label="Valor comercial" valueUf={result.propertyPriceUf} currencyMode={currencyMode} ufValue={effectiveUf} /><MoneyMetric label="Base financiable" valueUf={result.financingBaseUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Menor entre precio y tasación" /><MoneyMetric label="Capital financiado" valueUf={result.principalUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Calculado" /><MoneyMetric label="Pie mínimo" valueUf={result.requiredDownPaymentUf} currencyMode={currencyMode} ufValue={effectiveUf} /><MoneyMetric label="Gastos operacionales" valueUf={result.operationalExpensesUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Sin corretaje" /><MoneyMetric label="Corretaje del comprador" valueUf={result.buyerBrokerageUf} currencyMode={currencyMode} ufValue={effectiveUf} note={result.brokeragePlan?.includedInInitialCash ? "Incluido en efectivo inicial" : "No incorporado"} /><MoneyMetric label="Otros gastos iniciales" valueUf={result.otherInitialExpensesUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Supuestos editables" /><MoneyMetric label="Efectivo inicial total" valueUf={result.requiredInitialCashUf} currencyMode={currencyMode} ufValue={effectiveUf} /><MoneyMetric label="Pago capital e interés" valueUf={result.basePaymentUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Pago nivelado, salvo ajuste final" /><MoneyMetric label="Desgravamen inicial" valueUf={result.firstLifeInsuranceUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Estimado" /><MoneyMetric label="Incendio y sismo" valueUf={result.firstPropertyInsuranceUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Estimado" /><MoneyMetric label="Primer pago total" valueUf={result.firstTotalPaymentUf} currencyMode={currencyMode} ufValue={effectiveUf} /><Metric label="Carga hipotecaria" value={formatPercent(result.burdenRatio)} note="Pago total / ingreso" /><Metric label="Carga total" value={formatPercent(result.totalDebtBurdenRatio)} note="Incluye obligaciones existentes" /><Metric label="Ingreso orientativo" value={formatClp(result.requiredIncomeClp)} note="Según umbral elegido" /><Metric label="Margen mensual" value={formatClp(result.monthlyMarginClp)} note="Dentro del umbral elegido" /><MoneyMetric label="Intereses totales" valueUf={result.totalInterestUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Durante todo el plazo" /><MoneyMetric label="Seguros totales" valueUf={result.totalInsuranceUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Bajo supuestos ingresados" /><MoneyMetric label="Desembolso total" valueUf={result.estimatedTotalOutflowUf} currencyMode={currencyMode} ufValue={effectiveUf} note="Pie + gastos + pagos mensuales" /><Metric label="Tasa mensual equivalente" value={formatPercent(result.monthlyRate, 4)} note={result.rateConvention === RATE_CONVENTIONS.EFFECTIVE ? "Desde tasa anual efectiva" : "Desde tasa nominal anual"} /><Metric label="Tasa anual equivalente del flujo" value={result.equivalentFlowRate ? formatPercent(result.equivalentFlowRate.effectiveAnnualized) : "No disponible"} note="Estimación del flujo; no es CAE oficial" /></dl>
    ...
    <aside className="contextual-cta"><div><h2>¿Quieres integrar esta decisión a tus finanzas personales?</h2><p>Puedo ayudarte a ordenar tu flujo de caja, evaluar tradeoffs y construir un plan financiero personal o familiar antes de asumir un compromiso hipotecario.</p></div><a className="button button--primary" href="/#finance" onClick={() => trackEvent("mortgage_finance_service_clicked", { destination: "personal_finance" })}>Conocer Finanzas personales <ArrowRight size={17} /></a></aside>
  </main></PageShell>;
}
